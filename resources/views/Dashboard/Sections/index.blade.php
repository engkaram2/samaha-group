...
@section('title', trans('back.section.sections'))
@section('breadcrumb')
    <div class="breadcrumb-line">
        <ul class="breadcrumb">
            ...
            {{--                </a>--}}
            {{--            </li>--}}
            <li class="active">@lang('back.section.sections')</li>
        </ul>
...
@section('content')
    ...
    <div class="panel panel-flat" dir="{{ direction() }}" style="margin: 20px;">
        <div class="panel-heading">
            @include('Dashboard.layouts.parts.table-header', ['collection' => $sections, 'name' => 'sections', 'icon' => 'sections'])
        </div>
        <br>
        <div class="list-icons" style="padding-right: 10px;">
            <a href="#" data-toggle="modal" data-target="#add_section"
               class="btn btn-success btn-labeled btn-labeled-left"><b><i
                        class="icon-plus2"></i></b>@lang('back.create-var',['var'=>trans('back.section.section')])
            </a>
        </div>

        @if($sections->count() > 0)
            <table class="table datatable-button-init-basic" id="sections" style="font-size: 16px;">
                <thead>
                <tr>
                ...
                @foreach($sections as $section)
                    <tr id="row-{{ $section->id }}">
                        ...
                        <td class="text-center">
                            <a href="{{ $section->ImagePath }}" data-popup="lightbox">
                                <img src="{{ $section->ImagePath }}" alt="" width="80" height="80" class="img-circle">
                            </a>
                        </td>
                        <td class="text-center"><a href=""> {{ isNullable($section->$name) }}</a></td>
                        <td class="text-center">{{isset($section->created_at) ?$section->created_at->diffForHumans():'---' }}</td>
                        <td class="text-center">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-{{ floating('right', 'left') }}">
                                        <li>
                                            <a href="javascript:void(0);"
                                               data-toggle="modal"
                                               data-target="#edit_section_{{$section->id}}">
                                                <i class="icon-database-edit2"></i>@lang('back.edit')
                                            </a>
                                        </li>
                                        <li>
                                            <a onclick="sweet_delete('{{ route('sections.destroy', $section->id) }}', {{ $section->id }})"
                    ...
                    @include('Dashboard.Sections.edit_modal')
                @endforeach
                </tbody>
            </table>
        @else
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="table-responsive content-group">
                        <!-- Basic layout-->
                        <form action="{{ route('sections.store') }}" class="form-horizontal" method="post"
                              id="submitted-form"
                            ...
                            <div class="panel panel-flat">
                                <div class="panel-heading">
                                    <h5 class="panel-title">
                                        <i class="icon-plus2"></i></b>@lang('back.create-var',['var'=>trans('back.section.section')])
                                    </h5>
...
@section('scripts')
    @include('Dashboard.layouts.parts.ajax_delete', ['model' => 'section'])
@stop
